...
function gr_cronjob() {
    ...
    $query = $query.'(SELECT count(id) FROM gr_mails WHERE sent=0) as pendmails,';
            ...
            if (isset($provider[1]) && !empty($provider[1])) {
                ...
                if (isset($provider[0])) {
                    ...
                    if ($r[0] && $r[1] == 'register') {
                        ...
                        gr_prnt("<script type='text/javascript'> window.onunload = function(){ window.opener.location.reload();} </script>");
                        gr_prnt("<script type='text/javascript'> window.close(); </script>");
    ...
    if ($arg[0] === 'valid') {
        ...
        if ($arg[1]["ldt"] == 'user') {
            ...
            if ($scheck[0]['grjoin'] == 0 || $scheck[0]['grrole'] == 3 || !isset($GLOBALS["roles"]['groups'][7]) && $scheck[0]['grrole'] != 1 && $scheck[0]['grrole'] != 2 && $scheck[0]['grmessaging'] == 'adminonly') {
                if (!isset($arg[2]) || isset($arg[2]) && $arg[2] != 1) {
                    $list[0] = new stdClass();
                    $list[0]->gid = $arg[1]["id"];
                    $list[0]->nomem = 'refresh';
                    gr_prnt(json_encode($list));
        ...
        if (!empty(trim($arg[1]["msg"]))) {
            ...
            if (isset($arg[1]["gif"]) && isset($arg[1]["gfm"]) && isset($arg[1]["mtype"]) && isset($GLOBALS["roles"]['features'][17]) && $arg[1]["mtype"] == 'sticker') {
                ...
                if (!empty($arg[1]["gif"]) && !empty($arg[1]["gfm"])) {
                    ...
                    $gfcchk = '/http(s)?:\/\/(thumbs\.)*gfycat\.com\/.*/';
                    if (preg_match($tchk, $arg[1]["gif"]) && preg_match($tchk, $arg[1]["gfm"]) || preg_match($gfcchk, $arg[1]["gif"]) && preg_match($gfcchk, $arg[1]["gfm"])) {
                        $typ = 'gifs';
                        $gif = 1;
                    }
                }
        ...
        if (isset($GLOBALS["roles"]['files'][4])) {
            ...
            if (flr('upload', 'attachfile', $dir, $fn)) {
                fc('grfiles');
                for ($i = 0; $i < $total; $i++) {
                    $do['id'] = $fn.str_replace(' ', '_', $_FILES['attachfile']['name'][$i]);
                    $do['type'] = 'zip';
                    ...
                    $data["msg"] = $fnb;
                    $data["ldt"] = $arg[1]["ldt"];
                    $data['xtra'] = str_replace(' ', '_', $_FILES['attachfile']['name'][$i]);
        ...
        $query = $query.' FROM ((SELECT * FROM gr_msgs WHERE ';
        $msgconditions = '';
        $msgconditions = $msgconditions.'gid=:fullid ';
        if ($GLOBALS["default"]->sysmessages == 'disable') {
            $msgconditions = $msgconditions.'AND type<>"system" ';
        }
        if ($arg[1]["ldt"] == 'user' && $pmlist != 1) {
            $msgconditions = $msgconditions.'AND id > (SELECT IFNULL((SELECT MIN(CAST(v3 AS SIGNED)) FROM gr_options WHERE type="clearchat" AND v1=:uid AND v2=:loadid LIMIT 1),0)) ';
        }
        if (isset($arg[1]["from"]) && !empty($arg[1]["from"])) {
            $data['msgsfrom'] = $arg[1]["from"];
            $msgconditions = $msgconditions.'AND id > :msgsfrom ';
        } else {
            $msgconditions = $msgconditions.'AND type<>"like" AND type<>"logs" ';
        }
        if (isset($arg[1]["to"]) && !empty($arg[1]["to"])) {
            $data['msgsto'] = $arg[1]["to"];
            $perload = 5;
            $msgconditions = $msgconditions.'AND id < :msgsto ';
        }
        if (isset($arg[1]["uid"]) && !empty($arg[1]["uid"])) {
            $data['msgsuid'] = $arg[1]["uid"];
            $msgconditions = $msgconditions.'AND uid=:msgsuid ';
        }
        if (isset($arg[1]["search"]) && !empty($arg[1]["search"])) {
            $data['search'] = '%'.$arg[1]['search'].'%';
            $msgsearch = 'AND msg LIKE :search AND type<>"file" ';
            $filesearch = 'AND xtra LIKE :search AND type="file" ';
            $msgconditions = $msgconditions.$msgsearch.'OR '.$msgconditions.$filesearch;
        }
        if (isset($arg[1]["msid"]) && !empty($arg[1]["msid"])) {
            $data['msgid'] = $arg[1]["msid"];
            $msgconditions = $msgconditions.'AND id=:msgid ';
        }
        $query = $query.$msgconditions;
        ...
        if (!isset($arg[1]["to"]) && !isset($arg[1]["uid"]) && !empty($uid) && !isset($arg[1]["msid"]) && !isset($arg[1]["search"]) && $pmlist != 1) {
            ...
            foreach ($r as $v) {
                if ($v['type'] == 'like' || $v['type'] == 'unlike') {
                        ...
                        if ($arg[1]["ldt"] == 'group') {
                            ...
                            if (count($mentions[2]) > 0) {
                                ...
                                if (isset($GLOBALS["roles"]['users'][10])) {
                                    ...
                                    $qmen = 'SELECT name,id FROM gr_users WHERE id IN ("'.$mentions.'") ORDER BY id DESC';
                                }
                                $rqm = db('Grupo', 'q', $qmen);
                                foreach ($rqm as $ment) {
                                    if (isset($GLOBALS["roles"]['users'][10])) {
                                        ...
                                        $list[$i]->msg = str_replace('@'.$ment['id'], '<i class="vwp mentnd" no="'.$ment['id'].'">'.$ment['name'].'</i> ', $list[$i]->msg);
                                    }
                    ...
                    if ($v['type'] === 'audio') {
                        ...
                        $list[$i]->sfile = $v['xtra'];
                        if (mb_strlen($v['xtra']) > 16) {
                            $list[$i]->sfile = trim(mb_substr($v['xtra'], 0, 8)).'...'.mb_substr($v['xtra'], -8);
                        }
                        $list[$i]->sfile = $list[$i]->sfile;
                        $list[$i]->filext = 'expired';
                        $list[$i]->fetxt = '';
...
function gr_urlscrapper($url) {
    ...
    if (!empty($url)) {
        ...
        if (stripos($contentType, 'text/html') !== false) {
            ...
            if (stripos($url, 'youtube.com/') !== FALSE || stristr($url, 'youtu.be/')) {
                if (stristr($url, 'youtu.be/')) {
                    preg_match('/(https:|http:|)(\/\/www\.|\/\/|)(.*?)\/(.{11})/i', $url, $final_ID);
                    $r['image'] = 'https://i.ytimg.com/vi/'.$final_ID[4].'/maxresdefault.jpg';
                    $done['img'] = 1;
                } else {
                    @preg_match('/(https:|http:|):(\/\/www\.|\/\/|)(.*?)\/(embed\/|watch.*?v=|)([a-z_A-Z0-9\-]{11})/i', $url, $IDD);
                    $r['image'] = 'https://i.ytimg.com/vi/'.$IDD[5].'/maxresdefault.jpg';
                    $done['img'] = 1;
                }
            }
